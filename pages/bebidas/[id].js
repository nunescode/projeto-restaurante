...
const alterar = () => {
  ...
  useEffect(() => {
    if (query.id) {
      axios.get("/api/bebidas/" + query.id).then((resultado) => {
        console.log(resultado.data);
        const bebida = resultado.data;
        for (let atributo in bebida) {
          setValue(atributo, bebida[atributo]);
        }
      });
    }
  }, [query.id]);
  ...
  function salvar(dados) {
    axios.put("/api/bebidas/" + query.id, dados);
    push("/bebidas");
  }
