...
const index = () => {
  ...
  function getAll() {
    axios.get("/api/porcoes").then((resultado) => {
      setPorcoes(resultado.data);
    });
  }

  function excluir(id) {
    if (confirm("Deseja realmente excluir?")) {
      axios.delete("/api/porcoes/" + id);
      getAll();
    }
  }
  ...
  const adicionarAoCarrinho = async (item) => {
    try {
      ...
      console.log("Porção adicionada ao carrinho:", response.data);
    } catch (error) {
    ...
    <>
      <div className={styles.background}>
        <Page>
          <br />
          <br />
          <Link href="/porcoes/form" className="btn btn-success">
            <BsPlusSquareFill /> Adicionar Porção
          </Link>
          <br />
          <br />
          <Row md={4}>
