...
const alterar = () => {
  ...
  useEffect(() => {
    if (query.id) {
      axios.get("/api/porcoes/" + query.id).then((resultado) => {
        console.log(resultado.data);
        const porcao = resultado.data;
        for (let atributo in porcao) {
          setValue(atributo, porcao[atributo]);
        }
      });
    }
  }, [query.id]);
  ...
  function salvar(dados) {
    axios.put("/api/porcoes/" + query.id, dados);
    push("/porcoes");
  }
        ...
        <Page>
          <Form className="text-white">
            ...
            <div className="text-center">
              ...
              <Link className="ms-2 btn btn-danger" href="/porcoes">
